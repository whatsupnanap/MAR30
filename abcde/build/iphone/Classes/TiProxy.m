...
//Rare exceptions to indicate a bug in the _abcde code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_abcde = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_abcde]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.abcde.module"
