...
//Rare exceptions to indicate a bug in the _april2lecture code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_april2lecture = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_april2lecture]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.april2lecture.module"
