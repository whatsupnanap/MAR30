...
//Rare exceptions to indicate a bug in the _day4 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_day4 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_day4]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.day4.module"
