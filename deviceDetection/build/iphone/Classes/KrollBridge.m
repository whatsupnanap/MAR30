...
NSString * deviceDetection$ModuleRequireFormat = @"(function(exports){"
...
@implementation deviceDetectionObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_devicedetection);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create deviceDetection global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "deviceDetection" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_devicedetection = [[deviceDetectionObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_devicedetection];
	
	NSString *_devicedetectionNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _devicedetectionNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_devicedetection valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:deviceDetection$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.devicedetection.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.devicedetection.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
