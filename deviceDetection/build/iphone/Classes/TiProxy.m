...
//Rare exceptions to indicate a bug in the _devicedetection code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_devicedetection = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_devicedetection]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.devicedetection.module"
