...
//Rare exceptions to indicate a bug in the _game code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_game = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_game]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.game.module"
