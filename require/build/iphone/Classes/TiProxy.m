...
//Rare exceptions to indicate a bug in the _require code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_require = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_require]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.require.module"
