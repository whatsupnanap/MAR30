...
NSString * tanaka_nana_project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation tanaka_nana_project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_tanaka_nana_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create tanaka_nana_project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "tanaka_nana_project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_tanaka_nana_project2 = [[tanaka_nana_project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_tanaka_nana_project2];
	
	NSString *_tanaka_nana_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _tanaka_nana_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_tanaka_nana_project2 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:tanaka_nana_project2$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.tanaka_nana_project2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.tanaka_nana_project2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
